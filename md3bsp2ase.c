...
const char *get_surface_type(mapSurfaceType_t t) {
	switch(t) {
...
int convert_bsp(const char *in_name, FILE *in, char *out_name) {
	dheader_t *bsp;
	dmodel_t *model;
	dsurface_t *surf;
	dshader_t *shader;
	...
	for (i = 0, model = (dmodel_t *)(buf
		...
		for (j = 0, count = 0, surf = (dsurface_t *)(buf
			...
			if (skip_planar && little_long(surf->surfaceType) == MST_PLANAR)
				...
				printf("WARNING: cannot handle %s surfaces yet, skipping\n",
					get_surface_type(little_long(surf->surfaceType)));
				continue;
			}
			++count;
		}
		...
		for (j = 0, surf = (dsurface_t *)(buf
			...
			printf("\tProcessing surface #%d: type %s, %d vertices, "
				"%d indices\n",
				j, get_surface_type(little_long(surf->surfaceType)),
...
int main(int argc, char *argv[]) {
	...
	if (!strcasecmp(ext, "md3")) {
		...
		retcode = convert_bsp(argv[1], infile, argv[2]);
	else {
